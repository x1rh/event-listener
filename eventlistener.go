    ...
    go func() {
        ...
        for {
            ...
            case <-ticker.C:
                ...
                newestBlockNumber, err := el.client.BlockNumber(ctx)
                if err != nil {
                    slog.Error("failed to get newest block number", "err", err)
                    continue
                }
                if toBlock.Uint64() > newestBlockNumber {
                    toBlock = big.NewInt(int64(newestBlockNumber))
                    if el.Contract.BlockNumber.Uint64() > newestBlockNumber {
                        slog.Error("from block number > newest block number", "from block number", el.Contract.BlockNumber.Uint64(), "newest block number", newestBlockNumber)
                        continue
                    }
                }
